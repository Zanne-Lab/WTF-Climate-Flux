function [p1,sv] = forcing_minutes(b,p)
...
if b == 1
    data = load("DRO.txt"); 
    fprintf('DRO \n')
    sv    = 0.080777228; % Sky-view factor [0:1]
elseif b == 2
...
elseif b == 3
    data = load("MLES.txt"); 
    fprintf('MLES \n')
    sv    = 0.270330412; % Sky-view factor [0:1]
elseif b == 4
    data = load("MLRF.txt"); 
    fprintf('MLRF \n')
...
elseif b == 5
    data = load("PNW.txt");
    fprintf('PNW \n')
    sv    = 0.999994914; % Sky-view factor [0:1]
else
    data = load("STCK.txt");    
    fprintf('STCK \n')
    sv    = 0.875683356; % Sky-view factor [0:1]
end
...
r               = 0.0065;       % Radius of stick (0.0065) [m]
L               = 0.41;         % Length of stick (0.41) [m]
psi             = azi_angle;    % Solar azimuth angle (radians)
z1              = h/100;        % Height of the stick (m)
zh              = (h/100)*(3/4);% Zero plane displacement (m) 
...
for i = 1:length(fuel_moisture)
    if fuel_moisture(i) < 10
        fuel_err(i) = 1.0;
    elseif  10 <= fuel_moisture(i) && fuel_moisture(i) < 20
        fuel_err(i) = 1.5;
    elseif  20 <= fuel_moisture(i) && fuel_moisture(i) < 30
        fuel_err(i) = 2.2;
    elseif  30 <= fuel_moisture(i) && fuel_moisture(i) < 50
        fuel_err(i) = 3.0;
    else
        fuel_err(i) = 5.0;
    end
