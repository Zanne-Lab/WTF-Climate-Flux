...
function vf_ = fuel_model_wood(t,x,p,q,T_a,L_dn,K_d_diff,K_d_dir,k_d,alp_sha,u,q_a,q_sat,H_p,rain,m,tspan)
...
r           = q(3);   % Height/2 of stick [0.0065 m]
...
V_t   = L*L*2*r;                                      % Total volume of the wood log [m3] 
r_c   = 2*r*(1-f);                                    % Height of the core [m]
L_c   = L;                                          % Length/width of the core [m]
V_c   = L_c*L_c*r_c;                                % Volume of the core [m3] 
V_o   = V_t - V_c;                                  % Volume of the outer layer [m3] 
% a_o   = L*L + 2*L*r;                                % Surface area of the wood log [m2]
a_s   = 2*L*L + 2*r*L*4;                                % Surface area of the entire wood log [m2] 
...
% Conduction [J/(h)]
% ro_mid  = (r_c + r)/2; % Mid-point of the outer layer
rc_mid  = r_c/2;                                            % Mid-point of the core
k_s     = (g.*(0.1941 + 0.004064.*m_s*100) + 0.01864)*3600; % Bulk con- ductivity of the stick [J/m/(h)/K]
C       = L*L*k_s*(T_o-T_c)./(rc_mid); % Flux of heat between the two layers
...
% Diffusion [kg/(h)]
D = L*L*d_s*rho_s.*(m_o_1-m_c_1)./(rc_mid); % Rate of diffusion into the core from the outer layer [kg/h]

% Precipitation [kg/(h)]

% P_inc = min(rain.*r.*L,m_max.*rho_s.*V_o-max(0,m_o+(-a_o.*E-D)));  % Absorbed precipitation [kg/d]
P_inc = min(rain.*L.*L.*dv,m_max.*rho_s.*V_o-max(0,m_o));  % Absorbed precipitation [kg/h]
