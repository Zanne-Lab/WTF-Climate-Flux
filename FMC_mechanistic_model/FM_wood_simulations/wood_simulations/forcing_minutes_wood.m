function [p1,sv] = forcing_minutes_wood(b,p)
...
r               = 0.05/2;       % Height of log of wood [0.05 m]
L               = 0.09;         % Length/width of log of wood] 0.09
...
alp_sha     = (L*L+2*L*r*2).*csc(phi).*((1-((cos(phi)).^2).*((cos(psi)).^2)).^0.5) + 2*r*L.*cot(phi).*cos(phi); % **  
